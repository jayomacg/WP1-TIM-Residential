...
Energy service demands in the residential sector are driven by the total dwelling stock for non-archetype energy services or apartment, attached or detached dwelling stock for archetype energy services, which in turn is driven by population growth and the occupancy rate (the average number of people per dwelling). The residential stock projections up to 2040 are taken from Economic and Social Research Institute's (ESRI's) housing demand estimates \cite{BerginREGIONALLEVEL}.
...
%% Ask Ankita for more info about Demand Calculations!!
%Compound Annual Growth Rate (CAGR)
%The occupancy rate is expected to fall to 2.12 in 2051 \citep{PropertyIndustryIreland2019}, from 2.75 in 2016 (CSO).


%  Intermediate values are estimated using Equation \ref{eq:OR}.
% \begin{equation}
% \label{eq:OR}
%  OR= a+b*log(\Delta t)
% \end{equation}
...
Where VAR\_ACT are the activity levels of end-use technology (k) for demand category (i) in year (y). DM is the exogenous demand for that energy service for year, y. The choice of technology is minimised while also satisfying all constraints. 

All technologies and retrofits are archetype specific and some technologies deliver both water and space heating, while other technologies can only deliver one type of heating. In addition, some technologies use more than one fuel to meet demand. 
The list of space and water heating technologies in TIM, is summarised in 
Scope of technologies
\cite{JointResearchCentre2017Techno-economicEU}
Danish Energy Agency - Technology Catalogue
Granularity
Calculation is minised and obeies contrstaints



%Space Heating demand - BER space heating is only within certain months and assumes 21C heating..
%Thermostats are not widely used for temperature regulation in private dwellings in Ireland; instead, the domestic heating schedule was assumed to be 07.00–09.00 h and 17.00–23.00 h for the heating season of October to May inclusive, based on the guidelines issued by the Sustainable Energy Authority of Ireland (SEAI) [32]. The thermal load consists of both space heating and a baseline demand for sustaining water heating. For non-domestic premises, the thermal load is defined by office hours in line with the electrical demand. The domestic thermal loads consideorange in this study are listed in Table 2; the thermal load for the non-domestic
...
\begin{table}[htbp]
 ...
 Name & Description & Space  & Water  & Space  & Water   & Space  & Water \\
 \hline
 R-SH\_?\_KER\_N1 & Kerosene Boiler & \checkmark & \ding{55} & \checkmark & \ding{55} & \checkmark & \ding{55} \\
R-SW\_?\_KER\_N1 & Kerosene Boiler & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark \\
 \hline
 \end{tabular}
}
