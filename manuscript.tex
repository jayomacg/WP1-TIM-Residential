...
%and some models have been used to optimise Ireland's entire energy system \cite{Chiodi2013ModellingSystem}\cite{Connolly2021AalborgEnergy-System}, but very 

In 2014, Rogan et al. \cite{Rogan2014LEAPsSystem} used OSeMOYOS to explore residential energy service demands in Ireland
 LEAP writes an input file for OSeMOSYS, runs it and imports its results back into LEAP. Currently, the LEAP input file only contains the aggregated electricity demand as the main driver for the optimization within OSeMOSYS.
 the model examines the
 3 main sceanrios - exploring transtport, residential, services and industy. The reference sceanrio - 
REF - TFC increase by 6.7\% in 2020, base year is 2008. space and water heating TFC have decreased in share of total residential TFC from 82 to 77 \%, whereas lighting, cooking and appliances have increased in TFC from 18 to 23 \%. assume that 1.4 \% of existing dwellings per annum will undergo a shallow retrofit in the reference
case, corresponding to just below 300,000 dwellings

In the energy efficiency scenario, we assume that 800,000 residential dwellings will be retrofitted between 2009 and 2020, in line with government targets for the BEH scheme,  Other policies in the residential sector are building regulations 2011 and building regulations 2013 and CFL lighting,  The combined impact of these policies is a reduction in TFC of 9.9 \% compared to the reference scenario. 

The energy efficiency+scenario explores the impact of an ambitious retrofitting scenario beyond its current rate of implementation: retrofitbest is realised by retrofitting the same overall number of dwellings but undertaking a deeper, more comprehensive package of
energy efficiency measures on each, resulting in 7.2 \% savings compared to the energy efficiency scenario and 16.5 \% savings compared to the reference scenario.
  Insights - A challenging issue for energy modelling of policy is the
barriers and obstacles to investment, particularly in
residential energy efficiency measures such as split
incentives and information gaps, which lead to suboptimal
uptake of economically beneficial energy
efficiency improvements.


Thellufsen examining DH with EnergyPLAN and TIMES in 2019 \cite{Thellufsen2019ImplementingIreland}. 

In Ireland few models have used energy system optimisation model to explore the residential sector. The majority of previous modelling on the residential sector in Ireland has involved bottom-up residential sector only models, rather than energy system models specifically examining the residential sector. Nevertheless, the residential models have provided insights on input data and methodology approach used in Ireland, some of which are outlined below.

Early work by Healy and CLinch ---\cite{Clinch2000HousingMortality,Healy2002FuelIreland,Clinch2003ValuingModel,Clinch2000DomesticFailure,Clinch2001Cost-benefitEfficiency}
...
SEAI’s National Energy Modelling Framework (NEMF) is a suite of modelling tools used for modelling the Irish
energy system, both supply and demand side, to inform Irish Government energy policy and project Ireland’s
energy use based on these policie - https://www.seai.ie/publications/National-Energy-Projections-to-2030.pdf
The macro-economic model currently in use is called COSMO (Core Structural Model of the Irish Economy),
which creates a baseline forecast of energy consumption by sector (ESRI, 2017c). A range of sectoral models
are used to analyse both supply and demand side energy. Electricity supply is modelled using a commercial
power system software called PLEXOS47. A BioHeat model is used for techno-economic modelling of
bioenergy supply chains in Ireland for the electricity generation, transport and heat sectors48. There is also a
separate transport energy simulation model, which considers biofuels blending and the growth of low
emissions vehicles in the vehicle stock, including electric vehicles. In addition, separate modelling by an
energy-efficiency policy analysis tool (Unlocking model) informed the energy efficiency estimates included in
the NEMF49. 
...
%FROM TIM Paper
%TIM has been co-developed with the LEAP-Ireland model (Mac Uidhir et al., 2020), which is a bottom-up simulation modelof the Irish energy system which simulates the impact of different policy measures and targets on overall GHG emissions inIreland, with a particularly granular representation of transport and residential heat demand. Underlying data for the relevantsectors are shared between TIM and LEAP using a Data Repository and shared coding convention dictionary to improve the consistency between the models, give more robust analytical insights for policy and to share and exchange expertise between themodelling teams. This also facilitates multi-model approaches to energy systems modelling, which can make use of harmonisedhybrid frameworks coupling simulation and optimisation simultaneously (Rogan et al., 2014)
...
%With update technological and macroeconomic data, structured specifically to analyse for sectoral five-year carbon budgets towards 2030 and 2050, it is hoped TIM will provide further and more robust insights than previous Irish TIMES model. 
...
\label{LitR:RES}
The residential sector was developed in parallel with other sectors in TIM, and also been structured concurrently and similarly to the residential sector in the new Irish LEAP model, to allow more accessible soft-linking. Fig.\ref{fig:RES} shows the schematic overview of the residential sector in TIM, illustrating the flow of energy from fuel to demand.  
...
The ArDEM-SQL, which was described in Section \ref{Intro:ESOMIreland} is a simulation model built on IS EN 13790. ArDEM-SQL is used to clean the input data in TIM, by changing internal temperatures. The internal temperature is highly dependent on occupancy behaviour, a thermal comfort  thermal discomfort are higher among the over-65s than for other household \cite{Healy2002FuelIreland} - living room temperatues between 12 and 26...s  and therefore there is are large variations per BER rating.
The International Standard ISO 7730 [6] uses these PMV and PPD indices to predict the thermal sensation of people exposed to moderate thermal environments, as well as to specify acceptable thermal environmental conditions for comfort...
Despite this, it is
...
efficient dwellings i.e. band C.
C = 19.2, D = 18, E = 18.2 and F= 18.7 older occupants dublin Ho
...
Non-Archetype - lack of data
The fuel is calibrated to the 
...
While also accounting for more fuels and technological options, the residential sector in TIM is structured with around retrofitting, not vice-versa as previous versions, so that it is more aligned to ongoing climate mitigation investments. . The Non-Archetype Demands are so, because of lack of data 
...
heat output (Qt) and electricity input in a given time per-iod (Pt) is expressed asQt= COPPt, where COP is the average an-nual coefficient of performance for the heat pump
