...
The TIMES-Ireland Model (TIM) is a newly developed optimisation model for the Irish energy system. This paper describes the methodology applied to the development of the residential sector in TIM. Some results are analysed to provide input to discuss insights from the residential sector in TIM.
%\par The Irish residential sector has proved hard to decarbonise, yet recently Ireland has set one of the most ambitious climate targets into law, a 51 \% greenhouse gas reduction in 2030, compared to 2018. There is a need for a new up to date and state of the art model to provide insights on the optimal decarbonisation pathway for the Irish energy system. This paper describes the methodology applied to the development of the residential sector in the new TIMES Ireland Model (TIM). Results are analysed to provide input to discuss insights from the residential sector in TIM.
...
\begin{itemize}
    \item Section \ref{sec:Introduction} of this paper looks at Ireland's climate targets, energy policy, and distinctive residential sector. While also examining how researchers have previously used bottom-up residential models and Energy System Optimization Models (ESOMs) to analyse the residential sector.
...
Where the residential sector is directly responsible for 47\% of heat and 30\% of electricity GHG emissions. The residential sector would also be indirectly responsible for a share of transport GHG emissions, the residential sector is therefore a pivotal sector in decarbonising energy in Ireland. 
...
The ArDEM-SQL, which was described in Section \ref{Intro:ESOMIreland} is a simulation model built on space heating calculation EN 13790. ArDEM-SQL alters the primary energy consumption of the BER database by changing the internal temperatures. 
...
Zooming in on the residential fuel-mix gives further insights on the optimal pathways to achieve 2030 targets. Figure \ref{fig:ResFuel} shows this, with the base year being represented on the top row, the four Climate Bill compliant scenarios on rows 2-5, and the last two rows are not compliant with the Climate Bill. 
...
The methodology applied to the residential sector in TIM is transparent, up-to-date, and robust. The previous version of TIM, namely, the Ireland TIMES model, which was used for almost 10 years in Ireland, and was continuously tweaked to react to the ever changing climate policy, climate targets, and technological advancements. 
\par TIM is developed to provide insights on sectoral carbon budgets and 2030 and 2050 climate targets. As the first iteration of TIM, the core model is expected to be improved, while specific research will tweak the core model to specifically analysis certain sectors. 
The large increase in electricity may require some soft-linking with PLEXOS to investigate power generation operating constraints and reliability of supply. While LEAP is another powerful tool within the Energy Policy and Modelling Group of MaREI, which can specifically examine climate policy.
\par The model will require some future developments in the residential sector, district heating, and renewable gas will require improvements. The large growth of data centres expected in Ireland can provide waste heat to DH networks. While the capacity of biometheane in Ireland can effect the potential decarbonisation of natural gas, adding biomethane to the natural gas grid has not been accounted for in the model.
...
Tomás Mac Uidhir worked in parallel with the TIM residential sector development on the LEAP residential model development, and he also started a new Ireland-wide group, which I joined with the aim of filtering the BER database. Ankita Singh Gaur worked on the residential demands and provided some time to assist with the demand calculations. 
