...
In Ireland, previous modelling on the residential sector has often involved bottom-up residential models, rather than energy system models specifically examining the residential sector. Nevertheless, the residential models have provided insights on input data and methodology approach used in Ireland. In 2011 Dineen and Ó Gallachóir \cite{Dineen2011ModellingHeating} developed a residential bottom-up model to assess the impacts of measures in Ireland's National Energy Efficiency Action Plan (NEEAP). The model focused on energy efficiency of space and water heating by examining technology and building fabric efficiency. The building fabric efficiency of newly occupied dwellings developed takes into account the theoretical increase in energy efficiency due to improved 2008 and 2010 building regulations. Space heating demand accounted for 70\% of household energy consumption in newly built dwellings in 1997, down to just under 60\% in 2006 and the model is predicts a reduction to 27\% by 2015.The paper acknowledges retrofitting of existing stock will play a large role in meeting energy efficiency targets, however it is not examined. Nor is the affect of internal temperatures on energy efficiency improvements.  
\par In 2013 Ahern et al.\cite{Ahern2013StateStock} investigated thermal retrofit measures to the existing detached, oil centrally heated, rural housing stock. A base geometry according to age bands and thermal characteristics was created in the study. The heat load for a statistical occupancy and year was established. The default values used in some of the BER database variables where highlighted. For example,  there are assumed U-values when the roof insulation thickness is unknown, this is based on age, but there are no U-values for uninsulated roofs and a 2001 survey found 18\% of detached housing in Ireland had no roof insulation. The internal temperature assumed for all dwelling was 20$^{\circ}$C. Ahern et al. found that newer dwellings, consume the greatest amount of energy in kWh/annum than all the older dwellings, due to the fact that their large floor area. Concluding the majority of energy efficiency progress for thermal use in Irish dwellings is being offset. 


The reduction in heating costs and CO2 emissions for houses constructed prior to 1979 (coinciding with the introduction of building regulations) and around 26% for newer homes, thus offering a significant contribution (44%) to Ireland’s residential carbon abatement projections and hence in meeting the EU’s directives on energy and carbon.

through energy efficiency measures has been offset by the so called size effect. The
The model predicts a reduction of 65% in running costs and
CO2 emissions for house types A–E and a reduction of 26% in running costs and CO2 emissions for house types F–J from retrofit measures, and forecasts a total saving of 276 million euro and a CO2 abatement of 1.21 MtCO2. By
the
potential exists to reduce running costs and CO2 emissions by an average of 63% for housing constructed prior to the 1979 building regulations (house types A–E) and by 26% for newer housing. The ratio of amounts of CO2 which can be potentially saved to
the amount of money saved was calculated as 4.35 kgCO2/h. Nationally an annual saving of h183 million euro is theoretically achievable with a resultant saving of 0.80 MtCO2 arising from insulation retrofits, the replacement of single glazed windows and increasing air tightness. The
\cite{Ahern2015EnergySystems}\cite{Dineen2015ImprovedSource}\cite{Uidhir2020ResidentialIreland}\cite{Uidhir2020ImprovingDecisions} and some models have been used to optimise Ireland's entire energy system \cite{Chiodi2013ModellingSystem}\cite{Connolly2021AalborgEnergy-System}, but very few models have used energy system optimisation model to explore the Irish residential sector. In 2014, Rogan et al used OSeMOYOS to explore energy service demands \cite{Rogan2014LEAPsSystem}, and Thellufsen examining DH with EnergyPLAN and TIMES in 2019 \cite{Thellufsen2019ImplementingIreland}. 
